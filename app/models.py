...
import datetime
import jwt
from bson.objectid import ObjectId #ObjectId'yi burada import ettim fonksiyonların içinde değil
...
try:
    ...
    db = None #Bağlantı hatası durumunda db'yi None olarak ayarla uygulama çökmesin diye.



class User:
    def __init__(self):
        # db'nin None olup olmadığını açıkça kontrol ediyorum.
        
        self.collection = db.users if db is not None else None

    def create_user(self, username, password, face_embeddings):
        # Eğer MongoDB bağlantısı yoksa, kullanıcı oluşturamam.
    ...
    def get_user_by_id(self, user_id):
        # Koleksiyon yoksa direkt None döndür
        if self.collection is None: return None
        try:
    ...
    def update_last_login(self, user_id):
        # Koleksiyon yoksa bir şey yapma
...
class FailedLogin:
    ...
    def log_attempt(self, username, ip_address):
        # Koleksiyon yoksa bir şey yapmıyorum.
        if self.collection is None: return 
        log_data = {
            "username": username,
